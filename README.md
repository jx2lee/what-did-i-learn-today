...
- [x] 호돌맨 강의
  - 작성글 저장 2 - 클래스 분리 ~
  - 현재까지 작업한 내용 코드 작성 완료
  - 목적: 강의를 보면서 따라하지 않고 보고 나서 직접 구현해보기
- [] bigtop mpack stack 프로젝트 추가 조사
  - YARN 스타트가 되지 않은 문제는.. 호스트를 여럿 나눠서 진행해봐야겠다.
  - 우선은 SPARK 가 없어도 YARN 은 설치되어야한다. 내용은 찾아서 진행할 예정 (근무시간 포함)
- [] spring 복습

### 회고
- 미뤘던 daily log 를 작성하려고 한다.
- 틈날 때마다 까먹지 말고 챙기자.
